<template>
	<section class="paging">
		<a class="pagingFirst" @click="prevPage()" :disabled="pageData.page == 1"
			>처음</a
		>
		<ul class="pagingList">
			<li
...
export default {
	...
	computed: {
		startPage: function () {
			let value =
				Math.floor((this.pageData.page - 1) / this.defaultPages) *
					this.defaultPages +
				1;
			return value;
		},
		totalPage: function () {
			return (
				Math.floor(this.pageData.total / this.pageData.count) +
				(this.pageData.total % this.pageData.count == 0 ? 0 : 1)
			);
		},
